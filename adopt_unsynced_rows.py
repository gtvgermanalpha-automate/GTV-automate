...
def page_live(onbuy):
    """{sku: (price, stock)} - price so stale-price exposure is visible
    (user got an order at an outdated lower price, 2026-08-29)."""
    out = {}
    offset, limit = 0, 100
    while True:
        ...
        for it in items:
            ...
            if s and s not in out:
                try:
                    lp = float(str(it.get("price") or "").replace(",", "").strip() or 0)
                except (TypeError, ValueError):
                    lp = 0.0
                try:
                    lst = int(float(it.get("stock") or 0))
                except (TypeError, ValueError):
                    lst = 0
                out[s] = (lp, lst)
...
def main():
    ...
    for i, r in enumerate(rows):
        ...
        if target == sku:
            ...
            lp, lst = live[sku]
            try:
                sp = float(str(r.get("Selling Price (£)") or "").replace(",", "").strip() or 0)
            except (TypeError, ValueError):
                sp = 0.0
            gap = f" | STALE live {lp:.2f} vs sheet {sp:.2f}" if sp and lp and abs(sp - lp) > 0.02 else ""
            print(f"ADOPT row {rownum} {sku!r} (live, stock {lst}){gap}")
        else:
            rekeyed += 1
